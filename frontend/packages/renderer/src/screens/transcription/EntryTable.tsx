...
import {selectAudioPadding} from '../../redux/settingsSlice';
import {selectActiveLines, setLines} from '../../redux/entrySlice';
...
function EntryTable({audioPlayer}: {audioPlayer: Howl}) {
  ...
  const [timeOutList, setTimeOutList] = useState<Array<ReturnType<typeof setTimeout>>>([]);

  const [lineAudioProgress, setLineAudioProgress] = useState<number>(0);
  const [intervalNode, setIntervalNode] = useState<ReturnType<typeof setTimeout> | null>(null);
  const audioPadding = useAppSelector(selectAudioPadding);
  console.log(lineAudioProgress);
  // Editing
                    ...
                    onClick={() => {
                      if (lines) {
                        ...
                        const interval = setInterval(() => {
                          ...
                          const progress =
                            (currentTime - computedLineStart) /
                            (computedLineEnd - computedLineStart);
